...
const start = Date.now(); let itemOk = false
    while (Date.now() - start < 800) {
      ...
      if (item.exists()) {
        ...
        if (tempPath === 'temp_attachments/XYZ/manual.pdf') { itemOk = true; break }
      }
      await new Promise(r => setTimeout(r, 10)); await nextTick()
    }
