...
test('list projects returns project context without api tokens', function () {
    ...
    ShiftServer::actingAs($owner)->tool(ListProjectsTool::class, [
        ...
        ->assertOk()
        ->assertSee(['Example Portal', 'production', 'portal.example.test', 'SHIFT Owner'])
...
test('search tasks returns filtered task summaries without full descriptions', function () {
    ...
    ShiftServer::actingAs($owner)->tool(SearchTasksTool::class, [
        ...
        ->assertOk()
        ->assertSee([(string) $matchingTask->id, 'Broken billing export', 'Example Portal', 'high'])
...
test('mcp create task stores registered project environment metadata', function () {
    ...
    shiftMcpAs($user, ['mcp:read', 'mcp:write'])->tool(CreateTaskTool::class, [
        ...
        ->assertOk()
        ->assertSee(['MCP task with environment', 'development', 'portal.example.test']);
...
test('mcp edit task can set and clear registered project environment metadata', function () {
    ...
    shiftMcpAs($user, ['mcp:read', 'mcp:write'])->tool(EditTaskTool::class, [
        ...
        ->assertOk()
        ->assertSee(['MCP environment edit task', 'development', 'portal.example.test']);
...
test('mcp edit task preserves environment metadata when environment is omitted', function () {
    ...
    shiftMcpAs($user, ['mcp:read', 'mcp:write'])->tool(EditTaskTool::class, [
        ...
        ->assertOk()
        ->assertSee(['MCP status edit task', 'awaiting-feedback', 'development', 'portal.example.test']);
...
test('list notifications omits raw notification content and classifies links', function () {
    ...
    DB::table('notifications')->insert([
        ...
        'data' => json_encode([
            ...
            'url' => 'https://portal.example.test/shift/tasks?task='.$task->id,
            'thread_id' => 123,
        ], JSON_THROW_ON_ERROR),
        'read_at' => null,
        'created_at' => now(),
    ...
    ShiftServer::actingAs($user)->tool(ListNotificationsTool::class, [
        ...
        ->assertOk()
        ->assertSee([$notificationId, 'TaskThreadUpdated', 'consuming_project', 'portal.example.test', '/shift/tasks', 'external'])
